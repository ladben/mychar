function parseExpression(content, character, array = false) {
  ...
  // if content is only a constant
  if (!operator && !expressions.length && constants.length && !values.length) {
    if (array) {
      return null;
    }

    return Number(constants[0]);
  }
  ...
  if (operator && expressions.length && constants.length && !values.length) {
    ...
    if (operator === '+') {
      return (
        exp_values.reduce((acc, curr) => acc + curr) +
        const_values.reduce((acc, curr) => acc + curr)
      );
    }
    if (operator === '-') {
      return (
...
function decipherExpression(expression, character) {
  ...
  const decipheredExp_array = exp_array.map((exp) =>
    parseExpression(exp, character),
  );
